...
export const saveConfig = (config: MessageConfig): void => {
  ...
  try {
    ...
    if (isQuotaExceededError(error)) {
      console.warn(
        "localStorage quota exceeded for config, attempting compression..."
      );
      ...
      try {
        ...
        console.error(
          "Failed to save config even with compression:",
          compressError
        );
        showStorageWarning(
...
export const saveBlocks = (blocks: unknown[]): void => {
  ...
  try {
    ...
    if (isQuotaExceededError(error)) {
      console.warn(
        "localStorage quota exceeded for blocks, attempting compression..."
      );
      ...
      try {
        ...
        console.error(
          "Failed to save blocks even with compression:",
          compressError
        );
        showStorageWarning(
